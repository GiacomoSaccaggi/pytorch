...
// LUStreamScratch<T> (one slice per batch) is defined in LinearAlgebra.h so the
// host can size the allocation with a plain sizeof.
template <typename T>
...
kernel void luStreamUpdate(
  ...
  device LUStreamScratch<T>& scr = scratch[tgid.y];
...
kernel void luStreamPivot(
  ...
  device LUStreamScratch<T>& scr = scratch[tgid.x];
