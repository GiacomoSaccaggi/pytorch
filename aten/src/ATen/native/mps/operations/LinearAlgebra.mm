...
static void lu_factor_panel_encode(const Tensor& LU,
  ...
  if (uM > kStreamMinRows) {
    const int64_t perBatch =
        isComplex ? sizeof(LUStreamScratch<c10::metal::complex<float>>) : sizeof(LUStreamScratch<float>);
    scratch = at::empty({static_cast<int64_t>(B) * perBatch}, LU.options().dtype(kByte));
  }
